...
class AdviceList extends Component {
	...
	send = () => {
		...
		console.log('为你推荐的ID %o:', this.props.params.ID)
		...
		this.props.updateRecommends(updateInst)
	}
	...
	onChange = (e, record) => {
		...
		if (e.target.checked == true) {
			...
			console.log('取消后选入数组的数组内容%o', difference)
		}
	}
	//-------------------------------------------------
	...
	adviceListTable = () => {
		const columns = [
			...
			},
			{
				...
				render: (text, record, index) => (
					<span>
						<Popconfirm title='就凭你还想把我删除?你敢点确定！你点一个试试！' onConfirm={() => this.onDelete(record, index)}>
		...
		let tds = this.props.recommendEntities[this.props.params.ID]
	...
	render() {
		...
		console.log('recommend: %o:', this.props.recommendEntities[this.props.params.ID])
		console.log('recommend_id: %o', this.props.params.ID)
